...
L = np.loadtxt('output_files/south_buizert/L.txt')
L_interp = interp1d(age, L)
H = np.loadtxt('output_files/south_buizert/H.txt')
H_ts = np.arange(age[0], age.max(), 15.) 
H_Ls = L_interp(H_ts) / 1e3
B = np.loadtxt('output_files/south_buizert/B.txt')
